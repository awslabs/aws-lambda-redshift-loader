...
for r in `aws ec2 describe-regions --query Regions[*].RegionName --output text`; do aws s3 cp deploy-non-vpc.yaml s3://awslabs-code-$r/LambdaRedshiftLoader/deploy-non-vpc.yaml --acl public-read --region $r; done
